...
export const removeCountry = async (req: Request & { user?: any }, res: Response) => {
  try {
    const { code } = req.params;
    await SavedCountry.findOneAndDelete({ user: req.user._id, countryCode: code });
...
export const getSavedCountries = async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user || !req.user._id) {
      return res.status(401).json({ message: "Unauthorized: user not found" });
    }

    // Get saved records for the user
    ...
    // Extract all codes (handling if some saved entries have multiple codes in an array)
    const codes = saved
      .flatMap((item: { countryCode: string | string[] }) =>
        Array.isArray(item.countryCode) ? item.countryCode : [item.countryCode]
      )
    ...
    const { data } = await axios.get(
      `https://restcountries.com/v3.1/alpha?codes=${uniqueCodes.join(",")}`
    );
